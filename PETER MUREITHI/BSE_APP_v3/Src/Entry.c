...
// ========================
static const char zAppName[] = "Academy Merchant";                      // Application name
	...
	if (hDsp != NULL) {
		...
		do {
			if (usCt==0)                                                 // When counter==0, change image to display
			{
				...
				strcpy(tcPath, "file://flash/HOST/logo.png");
...
int keyboard_event(NO_SEGMENT noappli,S_KEY *key_in,S_KEY *key_out)
{
	...
	case N5:
		...
		MenuProcessingSelect(mnuSale);
		key_out->keycode=T_ANN;
		break;
	case N6:
		confirmGraphicLibHandle();
		MenuProcessingSelect(mnuPreaut);
		key_out->keycode=key_in->keycode;
		//key_out->keycode=T_ANN;
		break;
	case N7: case N8: case N9:
	case T_VAL:
	...
	case UP:
		confirmGraphicLibHandle();
		MenuProcessingSelect(mnuRefund);
		key_out->keycode=T_ANN;
		break;
	case DOWN:
		confirmGraphicLibHandle();
